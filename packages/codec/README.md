...
4. A new mnemonic needs a `summary`, an `example`, and the docs' prose (`use`, `idiom`, `pitfall`, and `see`) in `docs/notes.json`, and a family and its flags in `scripts/gen-opcode-docs.ts` (`FAMILY_OF`, `FLAGS`). Then `bun run opcodes` writes the repo's `docs/opcodes.json`, the reference the editor's hover cards and completions read, and the docs' language reference in `apps/web/src/docs/generated/reference/` (`scripts/gen-reference.ts`); `bun run docs-index` then rebuilds the docs' search index. `scripts/gen-opcode-docs.test.ts` fails until the file is current, and it runs every table row in the engine to hold `FLAGS` to what the engine does.
