...
export function Workspace({ layout, onLayout, panels, notify, className }: WorkspaceProps) {
  ...
  const boxes = useCallback(() => {
    ...
    for (const [id, slot] of slots.current) {
      if (slot.isConnected) out.set(id, slot.getBoundingClientRect())
    }
    return out
  }, [])
  ...
  const dropAt = (id: PanelId, x: number, y: number): Drop | null => {
    ...
    if (dock !== null) {
      const w = frame.width * DOCK_SHARE
      const h = frame.height * DOCK_SHARE
      const edge = {
    ...
    >
      ...
      {drag?.drop != null && (
        <div
          ...
          className="pointer-events-none absolute z-20 rounded-md border-2 border-accent bg-accent/15 transition-[left,top,width,height] duration-120 ease-out motion-reduce:transition-none"
          style={{
        ...
        return (
          <Fragment key={panelsOf(child)[0]}>
            <div
              ...
              style={{
                flex: fits(child) ? '0 0 auto' : `${node.weights[i] ?? 0} 1 0px`,
